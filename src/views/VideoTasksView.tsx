...
export const VideoTasksView: React.FC<VideoTasksViewProps> = ({
  ...
  const pendingTasks = filteredStaffTasks.filter(t => t.currentNode === 'appointment' || t.currentNode === 'pending_urgency');
  const inProgressTasks = filteredStaffTasks.filter(t => t.currentNode === 'shooting' || t.currentNode === 'editing');
  const reviewingTasks = filteredStaffTasks.filter(t => t.currentNode === 'manager_review');
  const completedTasks = filteredStaffTasks.filter(t => t.currentNode === 'finished');

  const getTabCount = (tab: string) => {
    switch (tab) {
      case 'pending':
        return staffTasks.filter(t => t.currentNode === 'appointment' || t.currentNode === 'pending_urgency').length;
      case 'in_progress':
        return staffTasks.filter(t => t.currentNode === 'shooting' || t.currentNode === 'editing').length;
      case 'reviewing':
        return staffTasks.filter(t => t.currentNode === 'manager_review').length;
      case 'completed':
        return staffTasks.filter(t => t.currentNode === 'finished').length;
      default: return 0;
    }
  };
  ...
  const handleCompleteCurrentNode = (task: TaskItem) => {
    if (task.currentNode === 'appointment') {
      onUpdateTaskNode(
        task.id,
        'shooting',
        {
    ...
    } else if (task.currentNode === 'editing') {
      onUpdateTaskNode(
        task.id,
        'manager_review',
        {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200">
          {[
            ...
            { id: 'in_progress', label: '制作中 (拍摄/剪辑)' },
            { id: 'reviewing', label: '负责人终审中' },
                ...
                sortedTasks.map((t, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 whitespace-nowrap text-right space-x-2">
                      ...
                      {t.currentNode !== 'finished' && t.currentNode !== 'manager_review' && (
                        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl max-w-xl w-full border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto text-xs">
              ...
              {/* Node 1: 待处理/预约 */}
              {processingTask.currentNode === 'appointment' && (
                <div className="space-y-4 pt-2">
                  <h4 className="font-bold text-slate-900 text-sm">【确认处理 / 预约拍摄】</h4>
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">预约开始拍摄时间 *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1">拍摄场地 / 影棚编号</label>
                    <input
                      type="text"
                      value={shootingScene}
                      onChange={(e) => setShootingScene(e.target.value)}
                      className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300"
                    />
                  </div>
                  <div>
              ...
              {/* Node 3: 剪辑 */}
              {processingTask.currentNode === 'editing' && (
                <div className="space-y-4 pt-2">
