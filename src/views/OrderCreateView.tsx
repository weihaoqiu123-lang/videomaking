...
import {
  ...
  FileText, Plus, Sparkles, Trash2, X, Zap,
} from 'lucide-react';
...
export const OrderCreateView: React.FC<OrderCreateViewProps> = ({
  ...
  const [addLandscape, setAddLandscape] = useState(false);
  ...
  const basePrice = calculateBasePrice({
    serviceId, tierId, formatId, packageId: ugcPackage, addLandscape, editingMode,
  ...
  const updateSku = (index: number, key: keyof SkuRow, value: string) => setSkuRows((rows) => rows.map((row, i) => i === index ? { ...row, [key]: value } : row));
  ...
  const applyServiceSwitch = (nextId: ServiceId) => {
    ...
    setUgcPackage('single'); setAddLandscape(false); setEditingMode('basic');
    setOutputVersionCount(1); setFinishedVideoCount(1); setSelectedPersonId('');
    setError(''); setSkuRows((rows) => [rows[0] || { sku: '', name: '' }]);
  };

  const formatSummary = serviceId === 'ugc'
    ? addLandscape ? '9:16 竖版 + 16:9 横版' : '9:16 竖版'
    : serviceId === 'custom' ? '由视频组确认' : formatNames[formatId];
  ...
  const validate = () => {
    if (serviceId !== 'custom' && (!skuRows[0]?.sku.trim() || !skuRows[0]?.name.trim())) return '请填写 SKU 和产品名称。';
    if (serviceId === 'ai_premium' && skuRows.some((row) => !row.sku.trim() || !row.name.trim())) return '请补全多 SKU 产品清单。';
  ...
  const handleSubmit = () => {
    ...
    const ratio: TaskItem['videoRatio'] = formatId === 'portrait' ? '9:16' : formatId === 'square' ? '1:1' : '16:9';
    ...
    onSubmitTask({
      ...
      outputFormatId: serviceId === 'custom' ? undefined : formatId,
      outputFormatName: formatSummary, quantity, basePrice,
      urgentApprovedPrice: isUrgent ? urgentPrice : null,
      serviceDetails: {
        ...details, sampleStatus, needsNarration, duration, ugcPackage, addLandscape,
      ...
      <main className="order2-layout"><div className="order2-form">
        <section className="order2-section" id="order-stage-1">
          ...
          <div className="order2-service-grid">
            {SERVICE_CATALOG.slice(0, 6).map((item) => <button type="button" key={item.id} className={serviceId === item.id ? 'is-active' : ''} onClick={() => item.id !== serviceId && setPendingServiceId(item.id)}><small>{item.number}</small><strong>{item.name}</strong><span>{item.priceLabel}</span>{serviceId === item.id && <Check size={16} />}</button>)}
          </div>
          <button type="button" className={`order2-custom-service ${serviceId === 'custom' ? 'is-active' : ''}`} onClick={() => serviceId !== 'custom' && setPendingServiceId('custom')}><span><small>07 / CUSTOM</small><strong>定制 / 专项需求</strong><em>不符合标准服务的项目，在确认 Brief 后报价</em></span><ChevronRight size={20} /></button>
        </section>
        ...
        <section className="order2-section" id="order-stage-2">
          ...
          {serviceId !== 'custom' && <div className="order2-sku-list">
          ...
          {serviceId === 'custom' && <div className="order2-fields"><Field label="项目描述 / Brief"><textarea value={details.projectBrief} onChange={(e) => updateDetail('projectBrief', e.target.value)} placeholder="描述项目背景、目标、交付内容和特殊要求" /></Field><Field label="品牌资料地址"><input value={details.brandAssetUrl} onChange={(e) => updateDetail('brandAssetUrl', e.target.value)} placeholder="品牌规范与素材地址" /></Field><Field label="参考作品"><input value={details.referenceUrl} onChange={(e) => updateDetail('referenceUrl', e.target.value)} placeholder="参考链接" /></Field><Field label="期望完成时间"><input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} /></Field></div>}
        ...
        <section className="order2-section" id="order-stage-3">
          ...
          {serviceId !== 'custom' && <div className="order2-spec-grid"><Field label="输出比例"><div className="order2-inline-choices">{config.formatIds.map((id) => <button type="button" key={id} className={(serviceId === 'ugc' ? id === 'portrait' : formatId === id) ? 'is-active' : ''} onClick={() => serviceId !== 'ugc' && setFormatId(id)} disabled={serviceId === 'ugc' && id === 'landscape'}>{formatNames[id]}</button>)}</div></Field><Field label="成片时长"><select value={duration} onChange={(e) => setDuration(e.target.value)}>{durationOptions.map((item) => <option key={item}>{item}</option>)}</select></Field></div>}
          {serviceId === 'ugc' && <label className="order2-checkline"><input type="checkbox" checked={addLandscape} onChange={(e) => setAddLandscape(e.target.checked)} />同时增加 16:9 横版，通过后按基础费用 ×1.5 结算</label>}
        ...
        <section className="order2-section" id="order-stage-4">
          ...
          <div className="order2-urgent"><div><Zap size={19} /><span><strong>申请加急</strong><small>提交期望完成时间，由视频组确认是否能够安排；若加急成功，则增加 20% 费用。</small></span></div><button type="button" className={isUrgent ? 'is-active' : ''} onClick={() => setIsUrgent((value) => !value)} aria-pressed={isUrgent}><span /></button>{isUrgent && <Field label="期望完成时间"><input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} /></Field>}</div>
